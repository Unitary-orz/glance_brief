...
- `topic` 是 4–8 字短导语，不写完整句子，不使用句末标点；renderer 会在最终 Markdown 中统一加粗 topic，semantic JSON 不要自行添加 Markdown 粗体。
- `summary` 是一条高密度事实摘要，目标 80–120 字，硬上限 140 字。保留主体、产品名、组织名、模型名、核心动作、结果，以及影响含义的日期、金额、数量和限定条件。
