...
- 将独立本地 GitHub 雷达 producer 纳入版本控制，和 `agents-report` 消费端保持清晰分层；根安装器不自动安装或创建其定时任务。
- 相关性筛选统一归一化连字符、下划线和空格，并增加可配置的编码 Agent 别名组，修复项目命名变体造成的漏召回。
- 增加相关性过滤诊断计数，并将 producer 的运行目录、配置、状态、输出和缓存路径改为可移植配置；不改变正式榜单结构。
- 将独立本地 GitHub 雷达收敛为可配置的 reader 接口：主报告只消费当天结构化快照和最终分类映射，不再在业务模块中写死 Hermes 路径或本地 Job ID。
- 从产品契约、架构、安装说明、Prompt、质量检查器和测试中统一开源热点板块规则：分类来自本地报告，每类只展示排名第一的项目，移除“其他项目”和独立“新发现”区域。
- 将旧版 agents-radar collector 改为显式兼容路径，默认报告不再在模块加载时探测或初始化其依赖。
