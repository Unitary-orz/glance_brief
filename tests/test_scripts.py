...
class NoonNewsTests(unittest.TestCase):
    ...
    def test_prompt_keeps_source_on_separate_line(self):
        ...
        self.assertIn("> 来源：[AIHOT](AI HOT条目链接)•[TechCrunch AI](原文链接)", prompt)
        ...
        self.assertIn("平台/渠道名与子板块、栏目或账号名之间用空格", prompt)
        self.assertIn("`公众号` 统一替换为 `WX`", prompt)
        self.assertIn("最多 2 个报告渠道", prompt)
        self.assertIn("+N", prompt)
        self.assertIn("同渠道去重", prompt)
        self.assertIn("分别作为两个 Markdown 链接", prompt)
        self.assertIn("[AIHOT](links.aihot)•[原文媒体](links.original)", prompt)
        self.assertIn("`TechCrunch：AI` / `TechCrunch•AI` → `TechCrunch AI`", prompt)
        self.assertIn("`WX・智谱（GLM）` / `公众号：智谱（GLM）` → `WX 智谱（GLM）`", prompt)
        self.assertIn("不同来源、渠道之间以及同一渠道的多个媒体之间使用 `•`", prompt)
        self.assertIn("不同来源、渠道之间以及同一渠道的多个媒体之间统一用 `•` 连接", prompt)
