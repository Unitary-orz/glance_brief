...
配置还保留 `ranking`：智力排行取 Top 2；均衡排行取 Top 2，计算范围为全部显式关注配置，不先排除已进入其他榜单的配置，使用 IQ、平均耗时、平均价格的百分位分数几何平均，平分时优先综合成本更低者；性价比 Top 3 只在非 Sol 显式关注配置中计算，先应用独立的 `IQ ≥ 70` 质量门槛，再按 `IQ / average_price_usd` 降序，时间和综合成本只作次级平分判断。这个门槛属于性价比排序，不等同于全局 IQ 筛选开关。榜单计算不互相排除；展示排版时，未进入任一 Top 榜的关注配置归入“其他”。综合成本沿用 CodexRadar 的价格/耗时权重；IQ 筛选阈值与这些排序分开，当前仍未启用。
