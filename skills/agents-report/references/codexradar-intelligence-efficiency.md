...
- `deepseek-v4.1-flash`：全部可用 effort
- `deepseek-v4-pro`：全部可用 effort
- `gpt-6-astra`：全部可用 effort
...
配置还保留 `ranking`：智力排行取 Top 2；均衡排行取 Top 2，计算范围为全部显式关注配置，不先排除已进入其他榜单的配置，使用 IQ、平均耗时、平均价格的百分位分数几何平均，平分时优先综合成本更低者；性价比 Top 3 只在非 Sol 显式关注配置中计算，先应用独立的 `IQ ≥ 70` 质量门槛，再按 `IQ × price_factor` 降序；`price_factor` 由 `ranking.value_price_bands` 提供，当前为 `≤$0.30 → 1.00`、`>$0.30–$0.50 → 0.98`、`>$0.50–$1 → 0.95`、`>$1–$3 → 0.78`、`>$3–$5 → 0.75`、`>$5 → 0.65`；超过 $1 后提高价格惩罚，时间和综合成本只作次级平分判断。非 Sol 候选为空时直接输出空榜，不回退引入 Sol；价格为 0 的配置作为免费项优先排序，并用 `value_is_free: true` 与 `value_score: null` 保持标准 JSON。这个门槛属于性价比排序，不等同于全局 IQ 筛选开关。榜单计算不互相排除；展示排版时，未进入任一 Top 榜的关注配置归入“其他”。综合成本沿用 CodexRadar 的价格/耗时权重；IQ 筛选阈值与这些排序分开，当前仍未启用。

“其他”不做 IQ 或性价比排名，按模型顺序 `Sol → Luna → DP V4.1 Flash → DS V4 Pro → Astra`，同一模型内按 effort 顺序排列，只用分号分隔。

展示层使用 `display.max_configs=12` 限制最终 Markdown 的模型配置展示条目总数（榜单重复出现也分别计数）；智力、均衡、性价比榜单的入选项必须保留，剩余名额按均衡分补足。未进入榜单且未被补足的关注配置不展示。
