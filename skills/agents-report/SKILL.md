...
- 分类标题文本必须逐字复用当天快照的 `local_report_categories`；展示时仅在标题前添加连续圆圈数字（`①`、`②`、`③`），不得根据项目描述、Topics 或模型知识自行分类、改名、合并、拆分或新建分类。
- “✨新热门开源”项目行严格使用本地雷达格式 `- [项目名](url)「一句话简介」(+X★/日)`，不添加逐行 `热门项目：`、`✨ `、`新入榜：` 或 `✨新热门开源：` 前缀；今日热门中的 fresh 项目仍按 `is_fresh_hot` 规则保留 `✨ ` 标记。
